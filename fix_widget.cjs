...
let content = fs.readFileSync('src/components/JarvisAssistant.tsx', 'utf-8');
...
const orb_orig = '<div className="relative flex flex-col items-center justify-center w-full flex-1">';
const orb_new = '<div className={`relative flex flex-col items-center justify-center w-full flex-1 ${isWidget ? "scale-[0.4] mt-2" : ""}`}>';
content = content.replace(orb_orig, orb_new);

// 8. Hide action toolbar in widget mode
const toolbar_orig = '<div className="absolute bottom-10 flex gap-4 p-3 rounded-full border border-white/5 bg-white/5 backdrop-blur-3xl shadow-2xl z-40 hover-glow">';
const toolbar_new = '{!isWidget && (<div className="absolute bottom-10 flex gap-4 p-3 rounded-full border border-white/5 bg-white/5 backdrop-blur-3xl shadow-2xl z-40 hover-glow">';
content = content.replace(toolbar_orig, toolbar_new);

const toolbar_end_orig = `          <button onClick={() => setIsVoiceModalOpen(true)} className="magnetic-btn p-4 rounded-full flex items-center justify-center bg-white/5 text-zinc-400 hover:text-white border border-white/10 cursor-pointer">
            <Sliders className="w-5 h-5" />
          </button>
        </div>
      </div>`;
const toolbar_end_new = `          <button onClick={() => setIsVoiceModalOpen(true)} className="magnetic-btn p-4 rounded-full flex items-center justify-center bg-white/5 text-zinc-400 hover:text-white border border-white/10 cursor-pointer">
            <Sliders className="w-5 h-5" />
          </button>
        </div>
        )}
...
const sys_log_orig = `              <span className="text-xs font-mono font-bold text-[var(--brand-light)] tracking-wider">
                SYS_LOG@CYBER_CORE:~
              </span>
            </div>
          </div>`;
const sys_log_new = `              <span className="text-xs font-mono font-bold text-[var(--brand-light)] tracking-wider">
                SYS_LOG@CYBER_CORE:~
              </span>
            </div>
          </div>
...
const input_form_orig = `        {/* Input area */}
...
const input_form_new = `        {/* Input area */}
...
fs.writeFileSync('src/components/JarvisAssistant.tsx', content, 'utf-8');
