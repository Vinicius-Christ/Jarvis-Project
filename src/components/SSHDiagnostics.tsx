  ...
  const diagnosticActions: ActionConfig[] = [
    {
      ...
      icon: Cpu,
      color: "border-cyan-800 text-[var(--brand-light)] bg-[var(--brand-dark)]/20 hover:border-[var(--brand-primary)]"
    },
    {
      id: "docker_prune",
      ...
      icon: HardDrive,
      color: "border-emerald-800 text-[var(--brand-light)] bg-[var(--brand-dark)]/20 hover:border-[var(--brand-primary)]"
    },
    {
      id: "purge_vram",
      ...
      icon: Terminal,
      color: "border-rose-800 text-[var(--brand-light)] bg-[var(--brand-dark)]/20 hover:border-[var(--brand-primary)]"
    },
    {
      id: "postgres_backup",
      ...
      icon: Database,
      color: "border-amber-800 text-[var(--brand-light)] bg-[var(--brand-dark)]/20 hover:border-[var(--brand-primary)]"
    }
  ];
      ...
      <div className="bg-zinc-900/35 border border-zinc-800 p-6 rounded-2xl">
        <div className="border-b border-zinc-800 pb-4 mb-6">
          <div className="flex items-center gap-2 mb-1">
            <Shield className="h-5 w-5 text-[var(--brand-light)] transition-opacity duration-300" />
            <h2 className="text-sm font-bold text-[var(--brand-light)] uppercase tracking-widest">
              ...
              >
                ...
                <div className="flex justify-end pt-1">
                  ...
                  >
                    {isRunningThis ? (
                      <>
                        <RefreshCw className="h-3 w-3 animate-spin text-[var(--brand-light)]" />
                        <span>Manutenção...</span>
                      </>
                    ) : (
                      <>
                        <Play className="h-3 w-3 text-[var(--brand-light)]" />
        ...
        <div className="bg-zinc-950 border border-zinc-900 rounded-2xl overflow-hidden shadow-[0_0_20px_rgba(16,185,129,0.06)]">
          ...
          <div className="bg-zinc-900 px-4 py-2 flex items-center justify-between border-b border-zinc-950 text-[10px] text-zinc-500">
            <div className="flex items-center gap-2">
              <span className="w-2.5 h-2.5 rounded-full bg-[var(--brand-primary)]/80"></span>
              <span className="w-2.5 h-2.5 rounded-full bg-[var(--brand-primary)]/80"></span>
              <span className="w-2.5 h-2.5 rounded-full bg-[var(--brand-primary)]/80"></span>
          ...
          <div className="p-4 bg-zinc-950 min-h-[220px] max-h-[350px] overflow-y-auto font-mono text-[11px] text-zinc-300 space-y-1.5 scrollbar-thin select-text">
            {logs.map((log, index) => {
              if (log.startsWith("vinicius@RyzenDesktop:~$")) {
                return (
                  <div key={index} className="text-[var(--brand-light)] font-semibold pt-1">
                    {log}
                  </div>
                );
              }
              if (log.includes("[SUCCESS]")) {
                return (
                  <div key={index} className="text-[var(--brand-light)] font-bold flex items-center gap-1.5">
                    <CheckCircle className="h-3 w-3 text-[var(--brand-light)] shrink-0" />
                    {log}
                  </div>
                );
              }
              if (log.includes("[ERRO]")) {
                return (
                  <div key={index} className="text-[var(--brand-primary)] font-bold flex items-center gap-1.5">
                    <AlertCircle className="h-3 w-3 text-[var(--brand-primary)] shrink-0" />
                    {log}
                  </div>
                );
              }
          ...
          >
            <span className="text-[var(--brand-light)] text-[11px] font-semibold px-2 shrink-0 select-none">
